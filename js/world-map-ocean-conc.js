document.addEventListener("DOMContentLoaded", function () {
    ...
    d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson").then(function (data) {
        ...
        // Tooltip formation
        let Tooltip = d3.select("#my_world_dataviz")
            .append("div")
            .attr("class", "tooltip");

        // Loading JSON 
        let masterData;
        d3.json("data/json/ocean_microplastic-conc-data.json").then(data => {
            console.log(data);
            masterData = data;
        ...
        let lassoPath, lassoPoints = [], isLassoing = false;
        ...
        svg.on("mousemove", (event) => {
            ...
            lassoPath.attr("d", "M" + lassoPoints.map(d => d.join(",")).join("L") + "Z");
        });
        ...
        svg.on("mouseup", () => {
            ...
            svg.selectAll("circle").each(function (d) {
                ...
                //const [cx, cy] = projection([d.long, d.lat]);
        ...
        oceanSelect.addEventListener("change", function () {
            const selectedOcean = this.value;
        ...
        function renderStackedBarChart(dataOcean, chartSvg) {
            ...
            const y = d3.scaleLinear()
                .domain([0, d3.max(dataOcean, d => keys.reduce((sum, k) => sum + d[k], 0))])
                .nice()
            ...
            chart.selectAll("g.layer")
                ...
                .on("mouseover", function (event, d) {
                    ...
                    Tooltip
                        ...
                        .style("color", "black")
                        .html(`<strong>${d.data.name}</strong><br>${type}: ${d.data[type]}`)
                        .style("left", `${event.pageX - 40}px`)
                        .style("top", `${event.pageY - 90}px`);
                })
                .on("mouseleave", () => Tooltip.transition().style("opacity", 0))
